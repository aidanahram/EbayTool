...
class LoginPage extends ReactiveWidget<LoginViewModel> {
  ...
  Widget build(BuildContext context, LoginViewModel model) => Scaffold(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              if (model.showSignUp)
                ..._signUp(context, model)
              else
                ..._signIn(context, model),
            ],
          ),
        ),
      );
  ...
  List<Widget> _signUp(BuildContext context, LoginViewModel model) => [
        Text((showSignUp ?? false) ? "Edit account" : "Create an account",
            style: context.textTheme.headlineLarge),
        const SizedBox(height: 24),
        SizedBox(
          width: 200,
          child: InputContainer(
              text: "First Name", controller: model.firstNameController),
        ),
        const SizedBox(height: 10),
        SizedBox(
          width: 200,
          child: InputContainer(
              text: "Last Name", controller: model.lastNameController),
        ),
